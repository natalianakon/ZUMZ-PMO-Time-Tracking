...
- **Dashboard — Capacity by Project panel** — New panel below Team Capacity showing hours by project vs. a 70%/30% target split (project work vs. admin/FY26). Each project row shows total hours, % of capacity, % of the 70% project target, and a relative bar. FY26 initiatives shown as a separate sub-section.
- **Dashboard — Project Spotlight** — Pick any project from the Dashboard to see total hours, period hours, breakdown by person, and breakdown by work type. Fixed a crash where `periodLabel` was undefined when loading the panel (hoisted to Dashboard-level `useMemo`).
- **Exec View — Fluent overdue fix** — `healthOf()` now checks status first: `Complete` → "Completed", `FY26` → "FY26 Year-Round", `On Hold` → "On Hold" — no overdue/at-risk logic runs on finished projects.
- **Release Notes — v2.1 updated** — Added "Capacity by Project" to v2.1 What's New.
- **Backup** — 4-file backup saved: JSX, JSON, CLAUDE, LOG (2026-05-12)
